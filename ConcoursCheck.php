...
$sessionEX=verify_donnees($_POST["sessionCON"]);
$nomEX=verify_donnees($_POST["nomConc"]);
$NomCAND=verify_donnees($_POST["NomCAND"]);
$Prenom=verify_donnees($_POST["Prenom"]);
$sexe=verify_donnees($_POST["Sexe"]);
$DateNais=verify_donnees($_POST["DateNais"]);
...
if(empty($sessionEX) || empty($nomEX)|| empty($NomCAND) || empty($Prenom) || empty($sexe) || empty($DateNais)||empty($lieu)|| empty($NumeroTb) || empty($Resultat) || empty($Moyenne)|| empty($Mention) ){
    echo "<h3 style='color:#ff3333;'>Veuillez remplir tous les champs svp</h3>";
    exit();

}else {
     if (!preg_match($Regdate,$DateNais)) {
    ...
    if ($reqs->fetch()) {
        echo "<h3>Ce Candidat existe déjà pour ce même concours! </h3>";
        exit();
    }else {
       
        $req=$databaseCon->prepare('INSERT INTO  resultats_concours(sessionCon,Concours,Num_Table,Nom_Candidat,Prenom_Candidat,Sexe,Date_Naissance,Lieu_Naissance,Resultat,Moyenne,Mention)VALUE(:sessionEx,:Examen,:Num_Table,:Nom_Candidat,:Prenom_Candidat,:Sexe,:Date_Naissance,:Lieu_Naissance,:Resultat,:Moyenne,:Mention)');
        $req->execute([
            'sessionEx'=>$sessionEX,
            'Examen'=> $nomEX,
            'Num_Table'=> $NumeroTb,
